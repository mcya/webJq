...
//        	alert("["+this.req.getResponseHeader('content-type').toLowerCase().substring(0, 9)+"]");
        	
        	if(this.req.getResponseHeader('content-type').toLowerCase().substring(0, 9) == 'text/html'
        		&& this.req.responseText && this.req.responseText.indexOf("登陆超时")!=-1){
        		//登陆超时
        		document.write(this.req.responseText);
        		return ;
        	}else if (this.req.getResponseHeader('content-type').toLowerCase().substring(0, 8) != 'text/xml')
...
AjaxAnywhere.prototype.handlePrevousRequestAborted = function() {
	...
	layer.msg('我已经很努力在加载了,请再等一下啦.', {icon: 5}); 
}
